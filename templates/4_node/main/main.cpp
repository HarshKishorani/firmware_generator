                                    ...
                                    IoT_Publish_Message_Params *params, void *pData)
{
    ...
    if (strcmp(param->string, "switch_1") == 0)
    {
        ...
        nodes.setDeviceState(1, param->valueint);
    ...
    */
    ...
    if (switchType == "RETRO")
    {
        ...
        while ((NETWORK_ATTEMPTING_RECONNECT == rc || NETWORK_RECONNECTED == rc || SUCCESS == rc))
        {
            ...
            if (device_config.fan && gpio_get_level(nodes.config.fan_switch) != nodes.fanSwitchState)
            {
            ...
            for (int i = 1; i <= nodes.config.size; i++)
            {
                ...
                int currentSwitchState = gpio_get_level(nodes.getSwitchGPIO(i));
                if (currentSwitchState != nodes.getLastSwitchState(i))
                {
                    currentSwitchState = gpio_get_level(nodes.getSwitchGPIO(i));
                    nodes.updateSwitchState(i, 0, currentSwitchState);
                }
            }
        }
    }
